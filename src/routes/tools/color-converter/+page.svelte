  ...
  <div class="mb-4 flex items-center justify-between flex-shrink-0">
    <div class="flex items-center gap-3">
      <div class="p-2 rounded-lg bg-accent-500/10">
        <Palette class="w-6 h-6 text-accent-500" />
      </div>
      <div>
    ...
    <button
      onclick={randomColor}
      class="flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
    >
  ...
  <div class="flex-1 overflow-auto">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div class="space-y-4">
        ...
        <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
          <div class="flex gap-4">
            <div
              class="w-32 h-32 rounded-xl shadow-inner border border-slate-200 dark:border-slate-700"
        ...
        <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
          ...
          <input
            ...
            class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-mono focus:ring-2 focus:ring-accent-500 focus:border-transparent"
          />
        </div>
        ...
        <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
          ...
          <div class="grid grid-cols-3 gap-2">
            <div>
              <label class="block text-xs text-slate-500 mb-1">R</label>
              <input
                type="number"
                min="0"
                max="255"
            ...
            <div>
              <label class="block text-xs text-slate-500 mb-1">G</label>
              <input
                type="number"
                min="0"
                max="255"
          ...
          <p class="mt-2 text-xs text-slate-500 font-mono">{rgbString}</p>
        </div>
        ...
        <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
          ...
          <div class="grid grid-cols-3 gap-2">
            ...
            <div>
              <label class="block text-xs text-slate-500 mb-1">S</label>
              <input
                type="number"
                min="0"
                max="100"
            ...
            <div>
              <label class="block text-xs text-slate-500 mb-1">L</label>
              <input
                type="number"
                min="0"
                max="100"
          ...
          <p class="mt-2 text-xs text-slate-500 font-mono">{hslString}</p>
        </div>
        ...
        {#if closestTailwind}
          <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
            <div class="flex items-center justify-between mb-2">
              ...
              <button
                onclick={() => copyValue(`${closestTailwind.name}-${closestTailwind.shade}`, 'tailwind')}
                class="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400"
              >
            ...
            <div class="flex items-center gap-3">
              <div
                ...
                style="background-color: {closestTailwind.hex}"
              ></div>
              <div>
      ...
      <div class="space-y-4">
        <!-- Generated Palette -->
        <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
          <h3 class="text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">Shades Palette</h3>
          <div class="flex rounded-lg overflow-hidden">
            {#each palette as shade}
              {@const rgb = hslToRgb(shade)}
              {@const hex = rgbToHex(rgb)}
              <button
                onclick={() => updateFromHsl(shade)}
                class="flex-1 h-12 transition-transform hover:scale-110 hover:z-10"
        ...
        <!-- Complementary -->
        <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
          <h3 class="text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">Complementary</h3>
          <div class="flex gap-2">
            {#each complementary as color}
              {@const rgb = hslToRgb(color)}
              {@const hex = rgbToHex(rgb)}
              <button
                onclick={() => updateFromHsl(color)}
                class="flex-1 h-16 rounded-lg transition-transform hover:scale-105 border border-slate-200 dark:border-slate-700"
                style="background-color: {hex}"
                title={hex}
              ></button>
            {/each}
        ...
        <!-- Triadic -->
        <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
          <h3 class="text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">Triadic</h3>
          <div class="flex gap-2">
        ...
        <!-- Analogous -->
        <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
          <h3 class="text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">Analogous</h3>
          <div class="flex gap-2">
            {#each analogous as color}
              {@const rgb = hslToRgb(color)}
              {@const hex = rgbToHex(rgb)}
              <button
                onclick={() => updateFromHsl(color)}
        ...
        <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
          <h3 class="text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">Tailwind Colors</h3>
          <div class="space-y-2 max-h-64 overflow-auto">
            {#each Object.entries(tailwindColors) as [name, shades]}
                ...
                <div class="flex-1 flex rounded overflow-hidden">
                  {#each Object.entries(shades) as [shade, hex]}
                    <button
                      ...
                      class="flex-1 h-6 transition-opacity hover:opacity-80"
                      style="background-color: {hex}"
                      title="{name}-{shade}"
                    ></button>
                  {/each}
