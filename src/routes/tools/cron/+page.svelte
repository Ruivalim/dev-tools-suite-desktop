<script lang="ts">
  ...
  function describeField(values: number[] | null, fieldIndex: number): string {
    ...
    if (values.length === max - min + 1) {
      return 'every ' + field.name.toLowerCase();
    }
    ...
    if (values.length > 2) {
      ...
      if (isStep && values[0] === min) {
        return `every ${step} ${field.name.toLowerCase()}${step > 1 ? 's' : ''}`;
      }
    }
  ...
  <div class="mb-4 flex items-center justify-between">
    <div class="flex items-center gap-3">
      <div class="p-2 rounded-lg bg-accent-500/10">
        <Clock class="w-6 h-6 text-accent-500" />
      </div>
      <div>
  ...
  <div class="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-4 min-h-0">
    ...
    <div class="flex flex-col gap-4">
      ...
      <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
        ...
        <div class="mt-1 grid grid-cols-5 gap-1 text-center text-xs text-slate-400 dark:text-slate-500 font-mono">
          <span>0-59</span>
          <span>0-23</span>
          <span>1-31</span>
          <span>1-12</span>
      ...
      <!-- Syntax Reference -->
      <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
        <h3 class="text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">Syntax Reference</h3>
        <div class="space-y-2 text-sm">
          <div class="flex gap-3">
            <code class="w-16 text-accent-500 font-mono">*</code>
    ...
    <div class="flex flex-col gap-4">
      ...
      <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
        <div class="flex items-center gap-2 mb-3">
          <Play class="w-4 h-4 text-accent-500" />
      ...
      <!-- Next Runs -->
      <div class="flex-1 p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
        <div class="flex items-center gap-2 mb-3">
          <Calendar class="w-4 h-4 text-accent-500" />
          ...
          <div class="flex-1 overflow-auto space-y-2">
            {#each nextRuns as run, i}
              <div class="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                <div class="flex items-center gap-3">
                  <span class="w-6 h-6 rounded-full bg-accent-500/10 text-accent-500 text-xs font-medium flex items-center justify-center">
                    {i + 1}
                  </span>
                  <span class="text-slate-900 dark:text-slate-100 font-mono text-sm">
                    {formatDate(run)}
                  </span>
                </div>
