...
<div class="flex h-full flex-col">
	...
	<div class="mb-4 flex items-center justify-between">
		<div class="flex items-center gap-3">
			<div class="rounded-lg bg-accent-500/10 p-2">
				<Fingerprint class="h-6 w-6 text-accent-500" />
			</div>
			<div>
	...
	<!-- Tabs -->
	<div class="mb-4 flex w-fit gap-1 rounded-lg bg-slate-100 p-1 dark:bg-slate-800">
		{#each ['uuid', 'ulid', 'nanoid'] as IdType[] as tab (tab)}
			<button
				onclick={() => switchTab(tab)}
				class={cn(
					'rounded-md px-4 py-2 text-sm font-medium transition-colors',
					activeTab === tab ? 'bg-white text-slate-900 shadow-sm dark:bg-slate-900 dark:text-slate-100' : 'text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100'
				)}
			>
				{tabInfo[tab].name}
			</button>
	...
	<!-- Controls -->
	<div class="mb-4 flex flex-wrap items-center gap-4 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
		<div class="flex items-center gap-2">
			<span class="text-sm text-slate-500">Count:</span>
			<div class="flex items-center overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700">
				<button onclick={() => (count = Math.max(1, count - 1))} class="px-2 py-1 hover:bg-slate-100 dark:hover:bg-slate-800">
					<Minus class="h-4 w-4" />
				</button>
				<input type="number" bind:value={count} min="1" max="100" class="w-16 border-x border-slate-200 bg-transparent px-2 py-1 text-center text-sm dark:border-slate-700" />
				<button onclick={() => (count = Math.min(100, count + 1))} class="px-2 py-1 hover:bg-slate-100 dark:hover:bg-slate-800">
					<Plus class="h-4 w-4" />
				</button>
			</div>
		</div>
		...
		{#if activeTab === 'uuid'}
			<div class="flex items-center gap-2">
				...
				<Select
					...
					onchange={(v) => {
						uuidFormat = v as typeof uuidFormat;
						reformatUUIDs();
					}}
					searchable={false}
					class="w-36"
		...
		{#if activeTab === 'nanoid'}
			<div class="flex items-center gap-2">
				...
				<input type="number" bind:value={nanoidSize} min="1" max="256" class="w-20 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-sm dark:border-slate-700 dark:bg-slate-900" />
			</div>
			<div class="flex items-center gap-2">
			...
			{#if nanoidAlphabet === 'custom'}
				<input
					...
					class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-sm dark:border-slate-700 dark:bg-slate-900"
				/>
			{/if}
		{/if}

		<button onclick={generate} class="flex items-center gap-1.5 rounded-lg bg-accent-500 px-4 py-1.5 text-sm font-medium text-white hover:bg-accent-600">
		...
		<button
			...
			class="flex items-center gap-1.5 rounded-lg bg-slate-200 px-4 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-300 disabled:opacity-50 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600"
		>
			<Plus class="h-4 w-4" />
			Add More
		</button>
		...
		{#if ids.length > 1}
			<button onclick={copyAll} class="flex items-center gap-1.5 rounded-lg px-4 py-1.5 text-sm text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">
				{#if copiedAll}
	...
	<div class="flex-1 space-y-2 overflow-auto">
		{#each ids as id, index (index)}
			<div class="group flex items-center gap-2 rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
				<span class="w-8 text-xs text-slate-400">{index + 1}.</span>
				<code class="flex-1 font-mono text-sm text-slate-900 select-all dark:text-slate-100">
					{id}
				</code>
				<button onclick={() => copyId(index)} class="rounded-md p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-slate-800 dark:hover:text-slate-300">
				...
				<button
					...
					class="rounded-md p-1.5 text-slate-400 opacity-0 transition-opacity group-hover:opacity-100 hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/20"
				>
					<Trash2 class="h-4 w-4" />
				</button>
			</div>
		...
		{#if ids.length === 0}
			<div class="flex h-full items-center justify-center text-slate-400">Click "Generate" to create IDs</div>
		{/if}
	</div>

	<!-- Stats -->
	{#if ids.length > 0}
		<div class="mt-4 flex items-center justify-between rounded-lg bg-slate-50 p-3 text-xs text-slate-500 dark:bg-slate-800/50 dark:text-slate-400">
