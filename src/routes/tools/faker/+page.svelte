<script lang="ts">
	...
	function updateFieldType(id: string, type: string) {
		...
		selectedFields = selectedFields.map((f) => (f.id === id ? { ...f, type, category } : f));
	}
...
<div class="flex h-full flex-col">
	...
	<div class="mb-4 flex items-center justify-between">
		<div class="flex items-center gap-3">
			<div class="rounded-lg bg-accent-500/10 p-2">
				<Sparkles class="h-6 w-6 text-accent-500" />
			</div>
			<div>
		...
		<div class="flex items-center gap-2">
			<button onclick={generateData} class="flex items-center gap-2 rounded-lg bg-accent-500 px-4 py-2 font-medium text-white transition-colors hover:bg-accent-600">
	...
	<div class="mb-4 flex flex-wrap items-center gap-4 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
		<div class="flex items-center gap-2">
			...
			<Select value={locale} options={localeOptions} onchange={handleLocaleChange} class="w-40" />
		</div>
		...
		<div class="flex items-center gap-2">
			...
			<input
				...
				class="w-20 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-900 transition-all focus:border-transparent focus:ring-2 focus:ring-accent-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
			/>
		</div>
		...
		{#if generatedData.length > 0}
			<button
				onclick={copyAsJson}
				class="flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-1.5 text-sm text-slate-700 transition-colors hover:bg-slate-100 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
			>
				{#if copied}
	...
	<div class="mb-4 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
		<div class="mb-3 flex items-center justify-between">
			...
			<button onclick={addField} class="flex items-center gap-1 text-sm text-accent-500 transition-colors hover:text-accent-600">
				<Plus class="h-4 w-4" />
				Add Field
			</button>
		</div>
			...
			{#each selectedFields as field (field.id)}
				<div class="flex items-center gap-2">
					<input
						...
						class="w-32 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-900 transition-all focus:border-transparent focus:ring-2 focus:ring-accent-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
					/>
					<Select value={field.type} groups={fieldGroups} onchange={(value) => updateFieldType(field.id, value)} class="flex-1" />
					<button
						onclick={() => removeField(field.id)}
						disabled={selectedFields.length <= 1}
						class={cn(
							'rounded-lg p-1.5 transition-colors',
							selectedFields.length > 1 ? 'text-slate-400 hover:bg-red-500/10 hover:text-red-500' : 'cursor-not-allowed text-slate-300 dark:text-slate-600'
						)}
					>
	...
	<!-- Data Table -->
	<div class="flex-1 overflow-auto rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
		{#if generatedData.length === 0}
			<div class="flex h-full flex-col items-center justify-center text-slate-400">
				<Sparkles class="mb-2 h-10 w-10 opacity-50" />
			...
			<div class="h-full overflow-auto">
				<table class="w-full text-sm">
					<thead class="sticky top-0 border-b border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800">
						<tr>
							<th class="w-12 px-4 py-2 text-left text-xs font-semibold tracking-wider text-slate-500 uppercase dark:text-slate-400">#</th>
							{#each selectedFields as field (field.id)}
								<th class="px-4 py-2 text-left text-xs font-semibold tracking-wider text-slate-500 uppercase dark:text-slate-400">
									{field.name}
								</th>
							{/each}
						</tr>
					...
					<tbody class="divide-y divide-slate-100 dark:divide-slate-800">
						{#each generatedData as row, i (i)}
							<tr class="transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50">
								<td class="px-4 py-2 font-mono text-xs text-slate-400 dark:text-slate-500">{i + 1}</td>
								{#each selectedFields as field (`${i}-${field.id}`)}
									<td class="px-4 py-2 font-mono text-slate-700 dark:text-slate-300">
										{row[field.name]}
									</td>
								{/each}
							</tr>
